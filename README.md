...
When you use ```pip install package_name``` you are installing it for the /usr/bin/python version, so when you run your python script and it loads a package, you will get a load error. Since your system default python is ```/usr/local/bin/python```.
